...
- [ ] **탭 및 보기 설정 (TabBar & Toggle) 구현**
  - [ ] "전체 언론사" / "내가 구독한 언론사" 탭 UI 구현
  - [ ] 구독한 언론사 수 뱃지 렌더링 및 탭 전환 로직
  - [ ] 리스트 뷰 / 그리드 뷰 토글 버튼 인터페이스 구현

## 2주차 (Week 2): 핵심 뷰(Grid/List) 구현, 상태 연동 및 마무리
- [ ] **전체 언론사 그리드 뷰 (Grid View) 구현**
  - [ ] 6x4 형태의 언론사 셀 레이아웃(총 24개) 및 `<PressGrid>` 구현
  - [ ] 페이지네이션(좌우 `<Chevron>` 화살표) 클릭 시 페이지 전환 로직
  - [ ] 언론사 셀 Hover 시 배경색 변경 및 `+ 구독하기` / `- 해지하기` 버튼 노출
- [ ] **기사 리스트 뷰 (List View / Opened Press) 구현**
  - [ ] 특정 언론사 선택 시 상세 뷰 전환 및 분야별 카테고리 탭 구현
